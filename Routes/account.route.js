...
router.post('/signUp', async function (req, res) {
  ...
  //Kiểm tra mail không trùng với mail cu
  var isCheck=0;
  const emails=await userModel.getUserEmail();
...
router.post('/profile', async (req, res) => {
  ...
  const ret = bcrypt.compareSync(req.body.passwordold, user.Password);
  if (ret === false) {
    alert('Pass cũ không đúng, vui lòng nhập lại')
